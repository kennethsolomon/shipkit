# /schema-migrate — Drizzle ORM Schema Change Analysis

**Invocation:** `/schema-migrate`

Analyzes pending Drizzle ORM schema changes on any project and provides safe, dialect-specific migration guidance. Supports **SQLite**, **PostgreSQL**, **MySQL**, and **Supabase**.

---

## Overview: 5-Phase Analysis

1. **Scan Schema State** — Read config, schema file, migration journal, git history
...
5. **Command Recommendations** — Dialect-specific workflows (db:push, db:migrate, supabase push)

---

## Phase 1: Scan Schema State
...
✅ **Read-only analysis** — skill never modifies files

✅ **No auto-execution** — never runs `db:push`, `db:generate`, `db:migrate`, or `supabase push` automatically

✅ **User confirms** — user must read and understand risk report before proceeding

✅ **Dialect-specific guidance** — recommendations tailored to SQLite, PostgreSQL, MySQL, or Supabase

✅ **Data safety first** — flags orphan rows, duplicates, and breaking changes before they cause data loss

---

## Testing This Skill

### Test Case 1: SQLite (db:push workflow, no pending changes)
```
Project: job-offer-aggregator
Expected output:
  • Dialect: SQLite
  • Workflow: db:push
  • Status: 0 pending changes
  • Warning: Migration snapshot out of sync (if true)
  • Recommendation: Run db:generate to sync snapshot
```

### Test Case 2: Supabase (db:migrate workflow, pending changes)
```
Project: any Supabase project
Expected output:
  • Dialect: PostgreSQL (Supabase)
  • Project: [name] (region: [region])
  • Status: X pending changes
  • Risk breakdown: green/yellow/red counts
  • Recommendations: db:generate → supabase push workflow
```

### Test Case 3: PostgreSQL (pending breaking change)
```
Project: any PostgreSQL Drizzle project
Pending change: column type change (text → integer)
Expected output:
  • 🔴 BREAKING change detected
  • Risk report with SQL migration path
  • Recommendation: Review migration before applying
```

### Test Case 4: MySQL (online DDL available)
```
Project: any MySQL 8.0+ Drizzle project
Pending change: index addition
Expected output:
  • Dialect: MySQL
  • Status: 🟢 SAFE change
  • Recommendation: db:migrate (uses ALGORITHM=INSTANT)
```
...
1. **Supabase remote schema sync**: Requires valid `SUPABASE_ACCESS_TOKEN` env var
   - Fallback: skip remote comparison if token unavailable

2. **RLS policy analysis**: Currently checks syntax only, doesn't validate auth context fully
   - User must manually review policy logic

3. **Custom SQL migrations**: Cannot parse non-Drizzle migration files
   - Assume user-written migrations are correct

4. **Type cast validation**: Cannot predict if PostgreSQL/MySQL CAST will fail
   - User must test migrations on staging first

5. **SQLite table recreation speed**: Not estimated; large tables may be slow
   - User should test on staging if concerned
