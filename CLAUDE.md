...
**a. Format/style/config/wording change** (formatter auto-fix, CSS tweak, copy change, config value, comment) → commit and re-run the gate. No test update needed.
...
**Why this matters:** Hard gates run after tests pass. If a gate requires a logic fix, tests become stale — they assert old behavior while the implementation has changed. This protocol ensures every logic change is tested before the gate re-runs.
