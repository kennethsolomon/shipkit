...
Custom skills and commands for **Claude Code** (default target) and **OpenAI Codex** (Phase 2 — in progress) for bootstrapping and maintaining projects with an enforced TDD workflow.
...
| **Package** | `@kennethsolomon/shipkit` v4.0.0-alpha.1 |
| **Testing** | Shell-based (`tests/verify-workflow.sh`) |
| **Targets** | Claude Code (shipped), OpenAI Codex (in progress) |

## Key Directories

| Path | Purpose |
|------|---------|
| `core/skills/` | Provider-agnostic skill implementations (`core/skills/*/SKILL.md`) |
| `core/commands/` | Provider-agnostic slash commands (`core/commands/sk/*.md`) |
| `adapters/claude/` | Claude Code emit logic (`emit.js`) — today's format is canonical |
| `adapters/codex/` | OpenAI Codex emit logic — Phase 2 stub today; full port tracked in `tasks/codex-migration-plan.md` |
| `tasks/` | Planning + findings + progress + security logs |
| `bin/` | CLI entry point (`shipkit.js`) — selects adapter via `--target=claude|codex|both` |
...
> **v4.0.0 refactor**: source content moved from `skills/` → `core/skills/` and `commands/` → `core/commands/` to enable multiple emit targets. `shipkit --target=claude` (default) produces byte-identical output to v3.30.0.

## Build & Run

```bash
npm install                              # install dependencies
node bin/shipkit.js                      # install for Claude (default)
node bin/shipkit.js --target=codex       # install for Codex (Phase 2 — not yet)
node bin/shipkit.js --target=both        # install for both
node bin/shipkit.js --uninstall          # remove install
...
**Write continuously:** `tasks/progress.md` (attempts/errors — always record what was tried, what failed, and what to try next), `tasks/findings.md` (discoveries), `tasks/cross-platform.md` (cross-platform impacts)
**Read before resuming:** Read `tasks/progress.md` before resuming any substantial in-progress session. Treat it as the lab notebook — append after meaningful progress, failed approaches, and blockers. Each entry must end with a `Next:` line stating what the next session should do first.
**Honest status labels:** When logging to `tasks/progress.md` or writing review findings, use explicit status — `verified` (confirmed working), `unverified` (not yet tested), `inferred` (assumed from evidence), `blocked` (cannot proceed without resolution), `ambiguous` (could mean multiple things — needs disambiguation before acting). Never smooth over gaps with vague language.
