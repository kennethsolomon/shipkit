...
**Honest status labels:** When logging to `tasks/progress.md` or writing review findings, use explicit status — `verified` (confirmed working), `unverified` (not yet tested), `inferred` (assumed from evidence), `blocked` (cannot proceed without resolution). Never smooth over gaps with vague language.
