...
def render_template(text: str, detection: Detection) -> str:
    replacements = {
...
def _write_file_if_generated(dest: Path, content: str) -> Tuple[str, Path]:
    if not dest.exists():
        dest.parent.mkdir(parents=True, exist_ok=True)
        dest.write_text(content, encoding="utf-8")
        return ("created", dest)
...
def apply(repo_root: Path, skill_root: Path, *, update_generated: bool) -> int:
...
def main(argv: List[str]) -> int:
    ...
    update_generated = "--update-generated" in argv
    if not repo_root.exists():
        print(f"Repo root not found: {repo_root}", file=sys.stderr)
        return 2

    return apply(repo_root, skill_root, update_generated=update_generated)
