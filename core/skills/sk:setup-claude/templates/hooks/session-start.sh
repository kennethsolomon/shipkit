...
# Current task — first unchecked item in tasks/todo.md, with staleness check
if [ -f "tasks/todo.md" ]; then
    CURRENT_TASK=$(grep -m1 '^\- \[ \]' "tasks/todo.md" 2>/dev/null | sed 's/^- \[ \] //')
    if [ -n "$CURRENT_TASK" ]; then
        # Check if todo.md was modified recently (within last 7 days)
        TODO_AGE_DAYS=999
        if command -v stat >/dev/null 2>&1; then
            if [ "$(uname)" = "Darwin" ]; then
                TODO_MTIME=$(stat -f %m "tasks/todo.md" 2>/dev/null)
            else
                TODO_MTIME=$(stat -c %Y "tasks/todo.md" 2>/dev/null)
            fi
            if [ -n "$TODO_MTIME" ]; then
                NOW=$(date +%s)
                TODO_AGE_DAYS=$(( (NOW - TODO_MTIME) / 86400 ))
            fi
        fi

        echo ""
        if [ "$TODO_AGE_DAYS" -gt 7 ]; then
            echo "Current task: $CURRENT_TASK (stale — todo.md last modified ${TODO_AGE_DAYS}d ago)"
        else
