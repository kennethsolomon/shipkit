...
2. **Isolate** — identify the failure location in code; narrow to smallest failing case
3. **Hypothesize** — form ONE specific hypothesis about root cause
4. **Verify** — test the hypothesis with minimal code (add a targeted log, write a unit test)
5. **Fix** — implement the minimal fix that addresses the root cause
6. **Verify fix** — confirm the original error is gone; run related tests

## Rules
- NEVER randomly change code hoping something fixes it — hypothesize first
- NEVER fix the symptom — fix the root cause
- 3-strike protocol: 3 different approaches all fail → stop and report what was tried and why each failed
- Add strategic debug logging when needed; remove all debug logs after the fix
