...
This symlinks the repo into `~/.claude/plugins/claude-skills`, which is where Claude Code discovers plugins. After this, **global skills** are immediately available in every project when you type `/` — no per-project setup needed.
...
- Renders templates from `~/.claude/plugins/claude-skills/skills/setup-claude/templates/`
...
python3 ~/.claude/plugins/claude-skills/skills/setup-claude/scripts/apply_setup_claude.py "$(pwd)"
```
